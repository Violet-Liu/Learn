...
namespace QZ.Service.Enterprise
{
    public class ResponseAdaptor
    {
        ...
        {
            var exhibit_list = new Resp_Exhibit_List() { count = sr.Total, exhibits = new List<SearchExhibit>(sr.Documents.Count()), cost = (sr.Took / 1000.0).ToString() };
            foreach (var hit in sr.Hits)
            {
                ...
                if (c != null)
                {
                    var se = new SearchExhibit();
                    se.e_count = c.e_count;
                    se.e_hall = c.e_hall;
                    se.e_name = c.e_name;
                    se.e_namemd = c.e_namemd;
                    se.e_showid = c.e_showid;
                    se.e_start = c.e_start.ToString("yyyy-MM-dd");
                    se.e_trade = c.e_trade;
                    se.e_year = c.e_year;
                    var hl = hit.Highlights;
                    foreach (var pair in hl)
                    {
                        se.hits.Add(pair.Key, pair.Value.Highlights.FirstOrDefault());
                    }
                    exhibit_list.exhibits.Add(se);
                }
            }
            // 统计
            if (sr.Aggregations.Count > 0)
        ...
        public static List<Company4FavorBrowse> CompanyFavorBrowse_Compose(List<Browse_Log> browses, List<Favorite_Log> favorites, int count,int u_id)
        {
            if (browses == null && favorites == null) return new List<Company4FavorBrowse>();

            if (browses == null) return favorites.Select(f => new Company4FavorBrowse(f.oc_code, f.oc_name, f.oc_area, f.favorite_date)).ToList();

            if (favorites == null) return browses.Select(f => new Company4FavorBrowse(f.oc_code, f.oc_name, f.oc_area, f.browse_date, true)).ToList();

            var list = new List<Company4FavorBrowse>();
            int bIndex = 0;
            int fIndex = 0;
            for(int i = 0; i < count; i++)
            {
                if(bIndex < browses.Count && fIndex < favorites.Count)
                {
                    var b = browses[bIndex];
                    var f = favorites[fIndex];
                    if (f.favorite_date.CompareTo(b.browse_date) > 0)
                    {
                        fIndex++;
                        if (!list.Exists(l => l.oc_code == f.oc_code))
                        {
                            var c = new Company4FavorBrowse(f.oc_code, f.oc_name, f.oc_area, f.favorite_date);
                            list.Add(c);
                        }
                    ...
                    else
                    {
                        ...
                        bIndex++;
                        if (!list.Exists(l => l.oc_code == b.oc_code))
                        {
                            var c = new Company4FavorBrowse(b.oc_code, b.oc_name, b.oc_area, b.browse_date);
                            list.Add(c);
                        }
                ...
                else if(fIndex < favorites.Count)
                {
                    var f = favorites[fIndex];
                    if(!list.Exists(l => l.oc_code == f.oc_code))
                    {
            ...
            list.ForEach(t => 
            {
                if (DataAccess.Favorite_Exsit_byUidAndOccode(t.oc_code, u_id))
                    t.fromBrowse = false;
            });
